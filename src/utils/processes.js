...
export function spawn(cmd: string, args: Array<string>,
                      opts: SpawnOptions = {}) {
  return limit(() => new Promise((resolve, reject) => {
                 ...
                 let cmdDisplayName =
                     opts.useBasename ? path.basename(cmd) : cmd;

                 let displayCmd = opts.disableCmdPrefix != null
                                      ? opts.disableCmdPrefix
                                      : globalOptions.get('disableCmdPrefix');
                 let cmdStr =
                     displayCmd ? '' : cmdDisplayName + ' ' + args.join(' ');
