...
jest.mock('../GlobalOptions',
          () => ({globalOptions : {setFromFlags : jest.fn()}}));
...
describe('cli', () => {
  ...
  describe('--help flag', () => {
    it('should run help command if --help flag is passed with no command args',
       async () => {
         const opts = {};
    ...
    it('should not run the help command if --help flag is passed WITH command args',
       async () => {
  ...
  describe('global flags', () => {
    ...
    it('should set global flags in globalOptions store', async () => {
      ...
      expect(globalOptions.setFromFlags).toHaveBeenCalledWith({prefix : false});
    });
  });
  describe('passing flags to run commands', () => {
    it('bolt script --flag1 --flag2', async () => {
      ...
      expect(mockedCommands.toRunOptions)
          .toHaveBeenCalledWith([ 'script' ], {flag1 : true, flag2 : true});
    ...
    it('bolt p run script --flag1 --flag2', async () => {
      ...
      expect(mockedCommands.toProjectRunOptions)
          .toHaveBeenCalledWith([ 'script' ], {flag1 : true, flag2 : true});
    ...
    it('bolt w my-package run script --flag1 --flag2', async () => {
      ...
      expect(mockedCommands.toWorkspaceRunOptions)
          .toHaveBeenCalledWith([ 'my-package', 'script' ],
                                {flag1 : true, flag2 : true});
      expect(commands.workspaceRun).toHaveBeenCalledTimes(1);
      expect(commands.workspaceRun).toHaveBeenCalledWith(opts);
    });
    it('bolt ws --only="my-package" run script -- --flag1 --flag2',
       async () => {
         const opts = {};
         mockedCommands.toWorkspacesRunOptions.mockImplementationOnce(() =>
                                                                          opts);
         await cli([
           'ws', '--only=my-package', 'run', 'script', '--', '--flag1',
           '--flag2'
         ]);
         expect(mockedCommands.toWorkspacesRunOptions)
             .toHaveBeenCalledWith(
                 [ 'script' ],
                 {'--' : [ '--flag1', '--flag2' ], only : 'my-package'});
         expect(commands.workspacesRun).toHaveBeenCalledTimes(1);
         expect(commands.workspacesRun).toHaveBeenCalledWith(opts);
       });
    it('bolt ws exec --only="my-package" -- yarn script  --flag1 --flag2',
       async () => {
         const opts = {};
         mockedCommands.toWorkspacesExecOptions.mockImplementationOnce(
             () => opts);
         await cli([
           'ws', 'exec', '--only=my-package', '--', 'yarn', 'script', '--flag1',
           '--flag2'
         ]);
         expect(mockedCommands.toWorkspacesExecOptions)
             .toHaveBeenCalledWith([], {
