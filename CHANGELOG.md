...
- Add --no-prefix flag to disable prefixing subcommand output with the command
  string
...
- `bolt add` on an existing dependency now correctly updates all workspaces
  versions if required (#251)
...
- Fixes bug where packages with a dep and a devDep on a package would not be
  able to upgrade that dep (#159)
...
- Fixes calculation of yarn relative path from package.dir instead of
  project.dir (#124)
